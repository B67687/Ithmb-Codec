...
use crate::types::{
    IGCodecApi, IGCodecCapability, IGHostApi, IGImageInfo, IGPixelBuffer, IGPluginApi, IGPluginInfo, IGStatus,
    IGStringRef, ig_status_from_decode_error,
};
...
/// ithmb magic bytes interpreted as a little-endian u32.
const ITHMB_MAGIC: u32 = 0x6d62_7468;
...
fn ensure_initialized() {
    ...
    let _ = PLUGIN_STATE.get_or_init(|| {
        ...
        let plugin_version = encode_utf16("0.1.0");
        ...
        let capability = IGCodecCapability {
            ...
            extensions: PLUGIN_EXTENSIONS.get().map_or(std::ptr::null(), |e| e.0.as_ptr()),
        };
...
unsafe extern "C" fn plugin_get_codec(_plugin: *const IGPluginApi, index: i32) -> *const IGCodecApi {
...
unsafe extern "C" fn plugin_initialize(_plugin: *const IGPluginApi, host: *const IGHostApi) -> IGStatus {
...
unsafe extern "C" fn codec_get_capability(_codec: *const IGCodecApi) -> *mut IGCodecCapability {
    let result = catch_unwind(|| -> *mut IGCodecCapability {
        PLUGIN_STATE
            .get()
            .map_or(std::ptr::null_mut(), |s| std::ptr::from_ref(&s.capability).cast_mut())
    });
...
unsafe extern "C" fn codec_can_handle_extension(_codec: *const IGCodecApi, ext: *const u16, len: i32) -> i32 {
    ...
    let result = catch_unwind(|| -> i32 {
        ...
        for known_ext in exts {
            ...
            #[allow(clippy::cast_sign_loss)]
            let known_slice = unsafe { std::slice::from_raw_parts(known_ext.data, known_ext.length as usize) };
...
/// Checks the first 4 bytes of a file for the ithmb magic.
unsafe extern "C" fn codec_can_handle_signature(_codec: *const IGCodecApi, data: *const u8, len: i32) -> i32 {
    ...
    let result = catch_unwind(|| -> i32 {
        ...
        // and we checked len >= 4 above.  Use read_unaligned because there
        // is no alignment guarantee for file-buffer pointers.
        unsafe {
            // SAFETY: caller guarantees `data` points to at least `len` bytes,
            // and we checked len >= 4 above.  Use read_unaligned because there
            // is no alignment guarantee for file-buffer pointers.
            let prefix = data.cast::<u32>().read_unaligned();
            i32::from(prefix == ITHMB_MAGIC)
        }
    });
...
unsafe extern "C" fn codec_decode_static_raster(
    ...
    let result = catch_unwind(|| -> IGStatus {
        ...
        let monitor = get_host_api().filter(|api| !api.core.is_null()).and_then(|api| {
...
unsafe extern "C" fn codec_free_pixel_buffer(_codec: *const IGCodecApi, buffer: *mut IGPixelBuffer) -> IGStatus {
    let result = catch_unwind(|| -> IGStatus {
        if buffer.is_null() {
            return IGStatus::InvalidArg;
        }
