...
//!
//! This is the most important file in the library. It orchestrates all decoders:
...
pub fn decode_ithmb(src: &[u8]) -> Result<DecodedImage, DecodeError> {
    if src.len() < 4 {
    ...
    decode_with_profile(src, &profile)
}
...
pub fn decode_with_profile(src: &[u8], profile: &Profile) -> Result<DecodedImage, DecodeError> {
    // JPEG data starts at offset 0 (the prefix bytes are part of the JPEG
    ...
    let img = dispatch_decode(frame_data, profile)?;
...
fn dispatch_decode(data: &[u8], profile: &Profile) -> Result<DecodedImage, DecodeError> {
    match profile.encoding {
        Encoding::Rgb565 => rgb565::decode(data, profile),
        Encoding::Rgb555 => rgb555::decode(data, profile),
        Encoding::ReorderedRgb555 => reordered_rgb555::decode(data, profile),
        Encoding::Yuv422 => {
            if profile.clcl_chroma {
                clcl::decode(data, profile)
            } else if profile.cl_chroma {
                cl::decode(data, profile)
            } else {
                uyvy::decode(data, profile)
            }
        }
        Encoding::Ycbcr420 => ycbcr420::decode(data, profile),
        Encoding::Jpeg => jpeg::decode(data, profile),
    }
}
...
mod tests {
    ...
    fn test_short_input_returns_buffer_too_short() {
        let result = decode_ithmb(&[0x00, 0x00, 0x00]);
        assert!(matches!(
            result,
    ...
    fn test_rgb565_dispatch() {
        ...
        let img = decode_with_profile(&buf, &profile).unwrap();
    ...
    fn test_reordered_rgb555_dispatch() {
        ...
        // Reordered RGB555 uses big-endian by default (little_endian=false).
        // White pixel: 0x7FFF in big-endian = [0x7F, 0xFF]
        let mut buf = Vec::new();
        buf.extend_from_slice(&9999i32.to_be_bytes());
        buf.extend_from_slice(&[0x7F, 0xFF]);
    ...
    fn test_jpeg_dispatch_with_decode_with_profile() {
        ...
        let result = decode_with_profile(&buf, &profile);
    ...
    fn test_decode_with_profile_too_short_for_prefix() {
        ...
        let result = decode_with_profile(&[0x00, 0x00], &profile);
        assert!(matches!(
            result,
    ...
    fn test_decode_ithmb_prefix_1007_dispatch() {
        ...
        let img = decode_ithmb(&buf).unwrap();
