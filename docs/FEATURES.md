...
> **Living artifact (mandated).** The project's complete reference of intended + confirmed features and how each is supposed to behave. It is the differential against which both problem-detection ("what deviated from intent?") and testing ("what should we prove?") are measured. Seeded at AMBITION, hardened to full behavior contracts at SPECIFICATION, maintained through every cycle. Updated in the same PR as the code that changes a feature.
>
...
- **Status:** applied
- **Reviewed:** 2026-08-20 (review cadence: 6 months)
