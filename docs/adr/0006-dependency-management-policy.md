...
3. **Tiny, stable dependency surface**: The workspace depends on only five crates (`jpeg-decoder`, `lru`, `thiserror`, `log`, `clap`, `anyhow`, `png`, `wasm-bindgen`, `pyo3`). An audit on 2026-08-01 showed everything current except trivial patch releases — there is no high-churn dependency problem to automate away.
